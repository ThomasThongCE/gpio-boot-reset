...
static int driver_probe (struct platform_device *pdev)
{
    ...
    for_each_child_of_node(np, child) {
        ...
        dev_num = MKDEV(dev_num_major, FIRST_MINOR + num_reset);
        device->dev = device_create_with_groups(data->dev_class, &pdev->dev, dev_num, device, device_groups, "%s", device->name);
